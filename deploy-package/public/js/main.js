...
class MainApplication {
     ...
     */
    async init() {
        window.logger?.log('开始初始化应用程序...');

        try {
            // 初始化各个模块
            window.logger?.log('1. 初始化模块...');
            await this.initializeModules();
            window.logger?.log('1. 模块初始化完成');

            // 绑定全局事件
            window.logger?.log('2. 绑定全局事件...');
            this.bindGlobalEvents();
            window.logger?.log('2. 全局事件绑定完成');

            // 加载数据
            window.logger?.log('3. 加载初始数据...');
            await this.loadInitialData();
            window.logger?.log('3. 初始数据加载完成');

            // 初始化UI组件
            window.logger?.log('4. 初始化UI组件...');
            this.initializeUI();
            window.logger?.log('4. UI组件初始化完成');

            this.isInitialized = true;
            window.logger?.log('应用程序初始化完成');
        } catch (error) {
        ...
        try {
            ...
            if (typeof UserManager !== 'undefined') {
                ...
                if (window.userManager.token) {
                    window.logger?.log('等待用户管理器初始化完成...');
                    // 等待会员信息加载完成
                    let attempts = 0;
                    ...
                    await new Promise(resolve => {
                        const checkInitialization = () => {
                            ...
                            if (window.userManager.membershipInfo !== null || attempts >= maxAttempts) {
                                window.logger?.log('用户管理器初始化完成，会员状态:', window.userManager.getMembershipStatus());
                                resolve();
                            } else {
                    ...
                    window.logger?.log('用户未登录，跳过会员信息加载');
                }
                window.logger?.log('用户管理器初始化完成');
            } else {
            ...
            if (typeof InteractionManager !== 'undefined' && window.userManager) {
                ...
                window.logger?.log('交互管理器初始化完成');
            } else {
                console.error('InteractionManager 类未找到或用户管理器未初始化');
                return;
            }

            window.logger?.log('所有模块初始化完成');
        } catch (error) {
    ...
    bindGlobalEvents() {
        ...
        document.addEventListener('themeChanged', (e) => {
            window.logger?.log('主题已切换为:', e.detail.theme);
        });
    ...
    initializeMobileComponents() {
        ...
        if (mobileSearchBar) {
            ...
            window.logger?.log('移动端搜索栏初始状态已设置为隐藏');
        }

        // 绑定汉堡菜单按钮事件
        if (hamburgerBtn) {
            hamburgerBtn.addEventListener('click', toggleHamburgerMenu);
            window.logger?.log('汉堡菜单按钮事件已绑定');
        }

        // 为移动设备优化点击事件
        this.optimizeForMobileDevices();

        window.logger?.log('移动端组件初始化完成');
    }
    ...
    optimizeForMobileDevices() {
        ...
        if (isMobileDevice) {
            window.logger?.log('检测到移动设备，优化触摸事件');
...
document.addEventListener('DOMContentLoaded', function() {
    window.logger?.log('页面加载完成，开始初始化应用程序...');

    // 检查是否已经初始化过
    if (window.mainApp) {
        window.logger?.log('主应用已经初始化过，跳过重复初始化');
        return;
    }
...
async function handleLike(event, novelId) {
    ...
    try {
        ...
        if (!window.userManager || !window.userManager.isLoggedIn()) {
            Utils.showToast('请先登录后再进行点赞操作', 'error');
            // 回滚UI
            if (isLiked) {
...
async function handleFavorite(event, novelId) {
    ...
    try {
        // 检查用户是否已登录
        if (!window.userManager || !window.userManager.isLoggedIn()) {
            Utils.showToast('请先登录后再进行收藏操作', 'error');
            // 回滚UI
            if (isFavorited) {
...
function initTheme() {
    // 主题管理器会自动处理初始化
    window.logger?.log('主题初始化由ThemeManager处理');
}
...
window.logger?.log('main.js 模块化重构完成');